...
function Home() {
  ...
  const [cardBooksWithExpiredDelivery, setCardBooksWithExpiredDelivery] =
    useState([]);
    ...
    axios
      ...
      .then((userRes) => {
        ...
        if (user.role === "profesor") {
          ...
          return axios
            .get(`/api/booksManager/getByUser/${user.id}`)
            .then((userBookCardsRes) =>
              userBookCardsRes.data.payload.bookCard.map((bookCard) => ({
                ...bookCard,
                expireDate: moment(bookCard.expireDate).format(),
                retiredDate: moment(bookCard.retiredDate).format(),
              }))
      ...
      .then((bookCardsData) => {
        ...
        const bookCardsConEntregaVencida = bookCardsData.filter((bookCard) =>
          moment(bookCard.expireDate).isBefore(currentDateTime)
        );

        if (bookCardsConEntregaVencida.length > 0) {
          const updatedList = bookCardsData.filter(
            (bookCard) =>
              !bookCardsConEntregaVencida.some(
                (vencido) => vencido._id === bookCard._id
              )
          );
          setList(updatedList);
  ...
  const deleteBookCard = async (bid) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await axios
          .delete(`/api/booksManager/${bid}`, { data: user })
          .then(() => {
            fetchData();
            Swal.fire(
              "¡Devuelto!",
              "Se confirmo la devolución con éxito",
              "success"
            );
          });
  ...
  const notifyUser = async (user) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        await axios.post("/api/users/notify", body).then((res) => {
          Swal.fire(
            "Enviado!",
            "¡El usuario fue notificado via Email con éxito!",
            "success"
          );
        });
      } else if (result.isDenied) {
    ...
    <div>
      ...
      {!isLoading && (
        <>
          ...
          <div id="list-th" className="books-manager-container mb-10">
            <div className="col-md-10 mb-2 mt-4 expired-area">
              {showAlert && cardBooksWithExpiredDelivery.length > 0 && (
                <div className="alert alert-danger flex flex-col items-center m-5">
                  <Alert
                    icon={<i className="bi bi-exclamation-triangle-fill"></i>}
                    variant="gradient"
                    color="amber"
                  >
                    La hora de entrega ha pasado para las siguientes tarjetas:
                  </Alert>
                  {cardBooksWithExpiredDelivery.map((booksCard) => (
                    <div className="book expired read bg-white" key={booksCard._id}>
                      <div className="cover">
                        <img src={booksCard.books.img} alt="imagen Libro" />
                      </div>
                      <div className="description">
                        <p className="leading-relaxed mb-3 font-bold title">
                          Usuario: {booksCard.owner.firstName}{" "}
                          {booksCard.owner.lastName}
                        </p>
                        <p className="leading-relaxed mb-3 bookTitle">
                          Libro: {booksCard.books.title}
                        </p>
                        <p className="leading-relaxed mb-3 text-sm horario">
                          {" "}
                          Fecha en la que se retiró:{" "}
                          {format(
                            new Date(booksCard.retiredDate),
                            "dd/MM/yyyy - HH:mm"
                          )}{" "}
                        </p>
                        <p className="leading-relaxed mb-3 text-sm horario">
                        ...
                        <div className="flex justify-center">
                          <Button
                            ...
                            color="red"
                            onClick={() => deleteBookCard(booksCard._id)}
                          >
                          ...
                          {user.role === "profesor" && (
                            <Button
                              className="mb-5 p-[10px] m-[10px] z-[1]"
                              color="yellow"
                              onClick={() => notifyUser(booksCard.owner)}
                            >
                              Notificar alumno
                            </Button>
                          )}
              ...
              {!isLoading && (
                <div>
                  ...
                  {list.length === 0 ? (
                    ...
                    list.map((booksCard) => (
                      <div className="book read bg-white" key={booksCard._id}>
                        <div className="cover">
                          <img src={booksCard.books.img} alt="imagen Libro" />
                        </div>
                        <div className="description">
